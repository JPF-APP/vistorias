...
async function gerarPdfVistoria(empresa, meta, ficha) {
  ...
  try {
    ...
    function titulo(texto, tamanho) {
      const fs = tamanho || 12;
      checkPageBreak(fs + 26);
      doc.setFont(undefined, "bold");
      doc.setFontSize(fs);
      doc.setTextColor(...PDF_RED);
      doc.text(texto, PDF_MARGIN, y);
      doc.setTextColor(20, 20, 20);
      const lineY = y + fs * 0.35 + 6; // um pouco abaixo do descendente do texto
      doc.setDrawColor(...PDF_RED);
      doc.setLineWidth(1);
      doc.line(PDF_MARGIN, lineY, pageW - PDF_MARGIN, lineY);
      y = lineY + 14; // espaço livre antes do conteúdo seguinte, para não sobrepor
    }
